...
def match_descriptors(desc1, desc2, method = "one_way", ratio_thresh=0.5):
    ...
    if method == "one_way": # Query the nearest neighbor for each keypoint in image 1
        ...
        # Step 2: Find the nearest and second nearest neighbors
        nearest_neighbors_indices = np.argmin(distances, axis=1)
        
        # Partition the distance matrix to find the second smallest value per row
        ...
        # Step 3: Compute the ratio between the nearest and second nearest distances
        ratio = nearest_distances / second_nearest_distances
        
        # Step 4: Apply the ratio threshold
        good_matches_mask = ratio < ratio_thresh
        
        # Step 5: Return the matches that pass the ratio test

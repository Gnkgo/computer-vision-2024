    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                # desc.extend(hist)
                hist = np.histogram(np.arctan2(grad_y[start_y:end_y, start_x:end_x], grad_x[start_y:end_y, start_x:end_x]), bins=nBins, range=(-np.pi, np.pi))[0]
                desc.extend(hist)
...
def create_bow_histograms(nameDir, vCenters):
    ...
    for i in tqdm(range(nImgs)):
        ...
        vBow_current = bow_histogram(vFeatures, vCenters)
